...
THREE SCOPES:
  1. GLOBAL (chats without folder):
     [IDENTITY] + [USER] + [FEEDBACK] from SQL (always injected)
     [PROJECT] from Vector DB semantic search (contextual, top-N)

  2. FOLDER (chats inside a folder — ISOLATED from global):
     [IDENTITY:folder:X] + [USER:folder:X] + [FEEDBACK:folder:X] from SQL
     [PROJECT:folder:X] from Vector DB (collection: folder-memory-{user_id}-{folder_id})
     Completely ignores real global memory.

  3. LOCAL (per-chat):
     [LOCAL:chat:X] from Vector DB (top-10 semantic + 5 most recent from SQL)
     Chat-specific decisions, solutions, architecture — retrieved via RAG.
...
class Filter:
    ...
    def _matches_scope(self, content: str, folder_id: str = None) -> bool:
        ...
        if folder_id:
            return f":folder:{folder_id}" in content.split("]")[0] if "]" in content else False
        else:
            # Global scope: no :folder: and no :chat: in the tag
            tag_part = content.split("]")[0] if "]" in content else ""
            return ":folder:" not in tag_part and ":chat:" not in tag_part
    ...
    async def _search_local_memories_vector(self, chat_id: str, query_text: str, request,
                                             limit: int = 10) -> list:
        """
        Semantic search for [LOCAL] memories in this chat's vector collection.
            ...
            if __request__:
                ...
                if latest_user_msg:
                    local_semantic = await self._search_local_memories_vector(
                        chat_id, latest_user_msg, __request__,
